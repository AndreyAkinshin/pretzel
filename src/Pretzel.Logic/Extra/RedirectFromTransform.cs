...
namespace Pretzel.Logic.Extra
{
    public class RedirectFromTransform : ITransform
    {
        ...
        private void WriteRedirectFile(SiteContext siteContext, Page post, IEnumerable<string> sourceUrls)
        {
            ...
            Tracing.Debug("REDIRECT: " + string.Join(", ", sourceUrls) + " -> " + targetUrl);

            foreach (var sourceUrl in sourceUrls)
            {
                try
                {
                    var filepath = _fileSystem.Path.Combine(siteContext.OutputFolder,
                        sourceUrl.TrimStart('/').Replace('/', _fileSystem.Path.DirectorySeparatorChar));
